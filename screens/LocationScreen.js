...
import { faArrowLeft, faCross, faPlus, faSearch, faMinusCircle } from '@fortawesome/free-solid-svg-icons';
...
const LocationScreen = (props) => {
    ...
    const [locationData, setLocationData] = useState([{ location: 'Ha Noi' }]);
    ...
    const getYourLocation = async () => {
        let temp = await getLocationData('locations');
        if (temp) {
                            ...
                            renderItem={({ item, index }) => {
                                return <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                                    ...
                                    <TouchableOpacity onPress={() => {
                                        let newLocationData = locationData.filter(remainItem => remainItem !== item)
                                        storeLocationData('locations', newLocationData)
                                        setLocationData(newLocationData)
                                    }}><FontAwesomeIcon icon={faMinusCircle} size={25} style={{ tintColor: 'black' }}></FontAwesomeIcon></TouchableOpacity>
